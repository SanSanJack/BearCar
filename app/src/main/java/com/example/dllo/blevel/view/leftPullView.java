...
public class leftPullView extends ViewGroup {
    ...
    public leftPullView(Context context) {
        super(context);
        this.context = context;
    }

    public leftPullView(Context context, AttributeSet attrs) {
        super(context, attrs);

        this.context = context;
    }

    public leftPullView(Context context, AttributeSet attrs, int defStyle) {
...
//        int measuredHeight = getMeasuredHeight();
        // 测量菜单页和首页
        // 根据布局文件中的角标获取子控件的对象
        //TODO 问题在这,对应的布局是 activity_main;
        menuView = ((ViewGroup) getChildAt(0));
        mainView = ((ViewGroup) getChildAt(0)).getChildAt(1);
        if (getChildAt(0) == null) Log.d("getChildAt", "getChildAt(0) is null ");
        if (menuView == null) Log.d("getChildAt", "menuView is null ");
        Log.d("leftPullView", "menuView:" + menuView);
        Log.d("leftPullView", "mainView:" + mainView);
    ...
    private void scrollToTime(int startX, int toX) {
        ...
        // dx, dy : 移动的距离
        // duration : 持续的时间
        scroller.startScroll(startX, 0, dx, 0, time);//移动到哪
        invalidate();
    }
